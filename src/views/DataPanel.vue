...
watchEffect(async () => {
    ...
    data.value = rawData.map((entry, i): Row => {
        return {
            ...
            date: moment.unix(entry.date).format(dateFormat),
        };
    });
...
watch(searchText, (text) => {
    table.value?.filter((row: Row) => {
        return row.expr.includes(text) || row.meaning.includes(text);
    });
});
...
const dateSortDirection = ref('asc');  // 初始排序方向
const dateSortClickCount = ref(0);     // 点击次数计数器
...
let collumns = reactive<DataTableColumns<Row>>([
    ...
    // 修改日期
    {
        title: "Date",
        key: "date",
        sorter(row1, row2, Format = dateFormat) {
        const date1 = moment(row1.date, Format);
        const date2 = moment(row2.date, Format);
